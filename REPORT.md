...
**Verify-as-Gemini-Pro is expensive.** The `verify` node uses Gemini 2.5 Pro to self-critique the prescribe draft, which costs one full Pro call per request even when the diagnosis is obviously correct. An alternative would be a cheap rule-based pre-check (does the answer cite at least one tool result? does the action match the diagnosis class?) that gates the Gemini Pro verify call. This would cut p95 latency by roughly 30%, at the cost of a more complex retry edge.
