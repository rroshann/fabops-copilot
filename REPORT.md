...
| 4 | **Trajectory tool-selection accuracy** | Measured from the `fabops_audit` spine on the 15 passing gold runs (verify-on config: `entry → check_policy → check_demand → check_supply → ground_in_disclosures → diagnose → prescribe → verify → finalize`, 9 nodes; 5 cases hit the verify-retry edge and ran 12). The deployed production fast path drops `verify` to 8 nodes; live `/monitor` traces confirm the expected order. *Why:* detects silent graph regressions where the agent completes but takes a wrong path | **100%** on passing runs |

**Operational metrics:**

| # | Metric | Definition · why it matters | Value |
|---|---|---|---|
| 5 | **End-to-end request latency** | Wall-clock seconds from API Gateway request to JSON response. *Why:* API Gateway HTTP API has a 30-second hard timeout; anything slower returns 504 to the user's browser even though the Lambda is still running. Latency is not vanity, it is a correctness boundary | **Gold-run (18 warm requests, verify-on): median 13 s, range 10–17 s** (two Gemini round-trips per request: `diagnose` and `verify`). **Live production aggregate from `/monitor` (163 requests, verify-off, mixed cold+warm): p50 31.8 s, p95 88 s** (one Gemini round-trip per request: `diagnose` only). Cold starts ~50 s; the gap between gold-run and live p50 is mostly cold starts and `__warmup__` pings rolled into the live aggregate |
| 6 | **Cost per request** | Gemini API spend per agent invocation plus DynamoDB read/write. Production (verify off): **Flash routing + Flash `diagnose` + DDB**. Verify-on path adds one more Flash call. *Why:* the $/request floor determines what a scaled deployment would cost. Every agent budget decision (Pro vs Flash, verify-on vs verify-off, DSPy compile vs not) is justified or rejected against this number | **~$0.002 per request** in production; ~$0.003 with verify on. Evaluation judge cost (separate) was $0.0354 across 18 cases ($0.00197/case) via Claude Haiku 4.5 |
...
**The frontend is intentionally vanilla HTML/JS.** This was a deliberate trade-off to keep the deploy surface small (one S3 bucket, one Amplify app) and avoid the React build chain entirely. It works, but the per-node streaming UI that would best showcase the agent's decision-making is harder to build in vanilla JS than in a small Next.js app. Given more time I would have picked Next.js to get the streaming animation cheaply.
